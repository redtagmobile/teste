...
Class ControllerLojas
{
    ...
    public function create(Login $l){
        $sql = 'INSERT INTO login_academia
    ...
    public function read($fk_loja){
        ...
        if(isset($_GET['idLoja'])){
            $idLoja = $_GET['idLoja'];
            $sql = "SELECT * FROM login_academia WHERE id_login_academia = '$idLoja'";
            ...
            $sql = 'SELECT * FROM login_academia WHERE fk_login_loja = ? AND fk_login_loja != id_login_academia';
    ...
    public function estadoLoja(Login $l){
        $sql = 'UPDATE login_academia SET 
        status = ?
        

          WHERE id_login_academia = ?';

        $stmt = Connect::getInstance()->prepare($sql);
        $stmt->bindValue(1, $l->getStatus());
        $stmt->bindValue(2, $l->getIdLoginAcademia());
        if($stmt -> execute()){
            session_start();
            $_SESSION['typeMessage'] = 'success';
    ...
    public function update(Login $l){
        $sql = 'UPDATE login_academia SET nome = ?, nome_loja = ?, email = ?, senha = ? WHERE id_login_academia = ?';
        $stmt = Connect::getInstance()->prepare($sql);
        $stmt->bindValue(1, $l->getNome());
        $stmt->bindValue(2, $l->getNomeLoja());
        $stmt->bindValue(3, $l->getEmail());
        $stmt->bindValue(4, $l->getSenha());
        $stmt->bindValue(5, $l->getIdLoginAcademia());
